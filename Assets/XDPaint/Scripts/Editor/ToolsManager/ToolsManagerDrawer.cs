...
namespace XDPaint.Editor
{
	...
	public class ToolsManagerDrawer : PropertyDrawer
	{
		...
		private void BeginDisabledGroup(object[] allAttributes, object obj)
		{
			...
			PaintToolConditionalAttribute attr = null;
			if (fieldPaintConditionalAttribute != null)
			{
				attr = (PaintToolConditionalAttribute)fieldPaintConditionalAttribute;
				var path = attr.Condition.Split(".".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
				if (path.Length > 1)
				{
					var nestedObj = obj;
						...
						if (memberInfo.Length > 0)
						{
							if (memberInfo[0] is FieldInfo field)
							{
								if (i == path.Length - 1)
								{
									...
									{
										isDisabled = (!Convert.ToBoolean(field.GetValue(nestedObj)) && !attr.Inverse) || (Convert.ToBoolean(field.GetValue(nestedObj)) && attr.Inverse);
									}
								}
								else
								{
							...
							else if (memberInfo[0] is PropertyInfo property)
							{
								if (i == path.Length - 1)
								{
									...
									{
										isDisabled = (!Convert.ToBoolean(property.GetValue(nestedObj)) && !attr.Inverse) || (Convert.ToBoolean(property.GetValue(nestedObj)) && attr.Inverse);
									}
								}
								else
								{
				...
				{
					attr = (PaintToolConditionalAttribute)fieldPaintConditionalAttribute;
					var member = obj.GetType().GetMember(attr.Condition, BindingFlags.Public | BindingFlags.Instance);
					if (member.Length > 0 && member[0] is FieldInfo conditionalField)
					{
						if (conditionalField.FieldType == typeof(bool))
						{
							isDisabled = (!Convert.ToBoolean(conditionalField.GetValue(obj)) && !attr.Inverse) || (Convert.ToBoolean(conditionalField.GetValue(obj)) && attr.Inverse);
						}
					}
					if (member.Length > 0 && member[0] is PropertyInfo conditionalProperty)
					{
						...
						{
							isDisabled = (!Convert.ToBoolean(conditionalProperty.GetValue(obj)) && !attr.Inverse) || (Convert.ToBoolean(conditionalProperty.GetValue(obj)) && attr.Inverse);
						}
					}
				}
			}
