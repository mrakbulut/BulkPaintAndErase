...
float4 BlendColors(float4 baseColor, float4 blendColor)
{
    float alpha = blendColor.a + baseColor.a * (1.0 - blendColor.a);
    if (alpha < 0.0001)
        return baseColor;

    float3 color = (blendColor.rgb * blendColor.a + baseColor.rgb * baseColor.a * (1.0 - blendColor.a)) / alpha;
...
float2 RotateCentered(float2 uv, float2 scale, float2 offset, float degrees)
{
    uv -= 0.5;
    uv *= scale;
    const float Deg2Rad = UNITY_PI * 2.0 / 360.0;
